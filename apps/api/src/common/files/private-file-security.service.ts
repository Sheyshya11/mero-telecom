...
import type { FileSecurityScanner, UploadedPrivateFile } from './private-file.types';
...
type AllowedMimeType = (typeof ALLOWED_PRIVATE_FILE_MIME_TYPES)[number];

@Injectable()
export class PrivateFileSecurityService implements FileSecurityScanner {
  async scan(file: UploadedPrivateFile) {
    const mimeType = detectMimeType(file.buffer);
    if (!mimeType || !ALLOWED_PRIVATE_FILE_MIME_TYPES.includes(mimeType)) {
